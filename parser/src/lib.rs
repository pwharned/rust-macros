...
pub fn list<'a, Output: 'a>(parser: Parser<'a, Output>, string: &'a str) -> Parser<'a, Arc<Vec<Output>>> {
...
pub fn aliased_table_name_identifier<'a>() -> Parser<'a, AliasedTableIdentifier>{
    table_name_identifier().and_then({  move|t| with_whitespace(name())  .or(Parser::new(|input, position| Ok(("", input, position)))).map(move |alias| AliasedTableIdentifier{alias: Some(alias.to_string()), identifier:t.clone() }  )}   )
}
...
name().and_then(move |table_name| match_char('.').and_then( move |_| name().map( move |column_name| JoinKey{table_name: table_name.to_string(),column_name:column_name.to_string()} ) ) )

}
pub fn join_segment_parser<'a>() -> Parser<'a, JoinSegment> {
    join_key_parser().and_then(move|l| with_whitespace(match_char('=')).and_then({  move |_| {
        
        join_key_parser().map({ let value = l.clone(); move |r| JoinSegment {left_key: value.clone(), right_key: r} }) }} ) )
}

pub fn join_parser<'a>() -> Parser<'a, Join> {

        aliased_table_name_identifier().and_then(move |at| join_list_parser().map( move |segments| Join {identifier: at.clone() ,join_segments: segments }  ) )
    
}

pub fn joins_parser<'a>() -> Parser<'a, Arc<Vec<Join>>> {

    with_whitespace(match_string("join")).and_then(move |_| 
    list(join_parser(), "join"))
}
...
pub fn select_parser<'a>() -> Parser<'a, Select> {
    with_whitespace(match_string("SELECT")).and_then({ move |_| select_column_list_parser(). and_then({move |cols| with_whitespace(match_string("FROM")).and_then({ move |_| aliased_table_name_identifier().and_then( {
        let cols = cols.clone();
        move |id| joins_parser().map({
            let cols = cols.clone();

            move |joins| Select{table_identifier: id.clone(), select_columns: cols.clone().to_vec(), joins:joins.to_vec() } })
...
pub fn select_column_parser<'a>() -> Parser<'a, SelectColumn> {
    select_column_with_alias_parser().or(column_name_table_name().map(move |col_name| SelectColumn{
        name: col_name.name.to_string(),
        table_name: col_name.table_name.clone(),
        alias: None
    } )  )
        ...
        for table in &mut tables {
            if constraint.0 .0 == table.0.name {
                match constraint.0 .1 {
                    ConstraintOrColumn::Constraint(ref a) => {
                        table.0.constraints.push(a.clone())
                    }
                    _ => continue,
                }
            }
...
pub fn build_table_hierarchy(select: &Select, tables: &HashMap<String, Table>) -> TableHierarchy {
    ...
    table_map.insert(select.table_identifier.identifier.name.clone(), hierarchy.clone());
    ...
    for join in &select.joins {
        ...
        let left_is_primary_key = is_primary_key(left_table, &join.join_segments[0].left_key.column_name, tables);

        // Check if the join key for the right table is a primary key
        let right_is_primary_key = is_primary_key(right_table, &join.join_segments[0].right_key.column_name, tables);
