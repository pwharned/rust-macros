            ...
            let result = #struct_name::insert(p,v ).await;
                  ...
                  match result {
                   ...
                   response.insert_header(("Content-Type", "application/json"));
                        ...
                        let js = #struct_name::from_row(&row).unwrap();
...
impl RouteGenerator for DeleteRoute {
    ...
    fn generate_test(&self) -> proc_macro2::TokenStream {
        ...
        let route = format!("/api/{}/{}", &self.table.name, pkey.join("/"));
...
impl RouteGenerator for GetAllRoute {
    ...
    fn generate_route(&self) -> proc_macro2::TokenStream {
        ...
        let get_schema_handler_function_name_syn = syn::Ident::new(
            &format!("{}_schema",&self.handler_function_name()),
            proc_macro2::Span::call_site(),
        );

        let route = self.route();
        let schema_route = format!("/api/schemas/{}", self.struct_name());
        let struct_name = self.struct_identifier();

        quote! {
                    #[get(#route)]
                    async fn #get_handler_function_name_syn(info: web::Query<#struct_name>, pool: web::Data<PgPool>) -> impl Responder {
                let v = info.into_inner();
        let res: Result<Vec<#struct_name>, sqlx::Error> = #struct_name::select(&pool, v).await;
 ...
 #[get(#schema_route)]
                    async fn #get_schema_handler_function_name_syn() -> impl Responder {
                let b = schema_for!(#struct_name);
                let mut response = HttpResponse::Ok();
                                response.insert_header(("content-type", "application/json"));
                                response.json(b)
...
#[proc_macro]
pub fn generate_structs_only(attr: TokenStream) -> TokenStream {
    ...
    let tables = parse::parse_ddl_file(relative_path);
       for stmt in tables {
        ...
        let struc = generate_struct_minimal(&struct_name, fields);
...
pub fn generate_structs_from_ddl(attr: TokenStream) -> TokenStream {
    ...
    #[async_trait]
    pub trait Select {
        async fn select(pool: &PgPool, value: Self) -> Result<Vec<Self>, sqlx::Error>
        where
            Self: Sized;
    }
 ...
 #[async_trait]
    pub trait SelectAll {
        async fn select_all(pool: &PgPool) -> Result<Vec<Self>, sqlx::Error>
        where
            Self: Sized;
    }
...
fn generate_impl(table: &parse::Table) -> proc_macro2::TokenStream {
    let basic_impl = generate_basic_impl(table);
    let delete_impl = generate_delete_impl(table);
    let update_impl = generate_update_impl(table);
    let select_where_impl = generate_select_where_impl(table);
    let select_impl = generate_select_impl(table, None);
    let insert_impl = generate_insert_impl(table);
    quote! {
        #basic_impl
        #delete_impl
        #update_impl
...
fn generate_delete_impl(table: &parse::Table) -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        impl Delete for #struct_name {
            async fn delete(pool: &PgPool, v: Self) -> Result<sqlx::postgres::PgQueryResult, sqlx::Error> {
                let query = format!("DELETE FROM {} WHERE {}", #tname, #where_cols);
...
fn generate_basic_impl(table: &parse::Table) -> proc_macro2::TokenStream {
    let tname = table.name.as_str();
    let struct_name = syn::Ident::new(to_camel_case(tname).as_str(), proc_macro2::Span::call_site());
    let where_clause = table.columns.iter().enumerate().filter(|col| !col.1. dtype.contains("[]") ).map(|(index,col)| {
        ...
        quote! {
            if !self.#field_name.is_none(){
                count+=1;
                fields.push(format!(" {} = ${} ",#colname, count ) );
            }
        }
    });
...
fn generate_insert_impl(table: &parse::Table) -> proc_macro2::TokenStream {
let tname = table.name.as_str();
    let struct_name = syn::Ident::new(to_camel_case(tname).as_str(), proc_macro2::Span::call_site());
 ...
 let insertable_fields = table
        ...
        .collect::<Vec<_>>();
    let return_fields = table.columns.iter().map(|x| { 
        match x.dtype.to_uppercase().as_str(){
            "UUID" => format!("cast({} as text) as {}", &x.name, &x.name),
            "TIMESTAMP" => format!("cast({} as text) as {}", &x.name, &x.name),
            "TIMESTAMPTZ" => format!("cast({} as text) as {}", &x.name, &x.name),
            _ => x.name.clone()
        }
   ...
   let non_default_bindings: Vec<proc_macro2::TokenStream> = table
        ...
        .map(|col| {
            ...
            match dtype {
                ...
                _ => quote! {
                    .bind(value.#field_name.as_ref().or(None) )
                },
            }
        })
        .collect(); 
    ...
    quote!{
                   ...
                   async fn insert(pool: &PgPool, value: Self) -> Result<Vec<sqlx::postgres::PgRow>, sqlx::Error> {
                           let placeholders: Vec<String> = (1..=#fields_length ).map(|i| format!("${}", i)).collect();
                            ...
                            let query = format!("INSERT INTO {} ({}) VALUES ({}) RETURNING {}", #table_name, #insertable_fields_string, placeholders.join(", "), #return_fields);
                            println!("{}",query);
...
fn generate_update_impl(table: &parse::Table) -> proc_macro2::TokenStream {
    let tname = table.name.as_str();
    let struct_name = syn::Ident::new(to_camel_case(tname).as_str(), proc_macro2::Span::call_site());
    let primary_keys: Vec<String> = table
        .constraints
    ...
    let inner_field_bindings = table
        ...
        .map(|col| {
            ...
            match dtype {
                "uuid" => quote! {
                    if value.#field_name.is_some() {
                        if!first{
                            sqlx_query.push(",");
                        }
                            sqlx_query.push(format!(" {} = ", #colname) ).push_bind( Uuid::parse_str(value.#field_name.as_ref().unwrap()).unwrap()   );
                            first = false;
                    }
                },
                _ => quote! {
                   ...
                   if value.#field_name.is_some(){ 
                        ...
                        sqlx_query.push(",");
                         ...
                         sqlx_query.push(format!(" {} =  ", #colname) ).push_bind(value.#field_name.as_ref().unwrap() );
                    first = false;
                    }
                },
            }
        });
    let where_bindings: Vec<proc_macro2::TokenStream> = table
        .columns
        .iter()
        .filter(|x| primary_keys.contains(&x.name))
        .enumerate().map(|(index, col)| {
            let field_name = syn::Ident::new(&col.name, proc_macro2::Span::call_site());
            let dtype = col.dtype.clone();
            let colname = &col.name;
            match index {
            
           0 =>  match dtype.as_str() {
                "uuid" => quote! {
                    sqlx_query.push(format!(" where {} = ", #colname)).push_bind( Uuid::parse_str(v.#field_name.as_ref().unwrap()).unwrap()   );
                },
                _ => quote! {
                    sqlx_query.push(format!(" where {} = ", #colname) ) .push_bind(v.#field_name.as_ref().unwrap() );
                },
            },
                _ =>  match dtype.as_str() {
                "uuid" => quote! {
                     sqlx_query.push(format!(" and {}  = ", #colname)).push_bind( Uuid::parse_str(v.#field_name.as_ref().unwrap()).unwrap()   );
                },
                _ => quote! {
                    sqlx_query.push(format!(" and {} = ", #colname) ) .push_bind(v.#field_name.as_ref().unwrap() );
                },
            },
    ...
    quote! {
        ...
        impl Update for #struct_name {
            async fn update(pool: &PgPool, value: Self, v: Self) -> Result<sqlx::postgres::PgQueryResult, sqlx::Error> {
                let pkeys = vec![#(#primary_keys),*];
                println!("Entering update");
                let active_fields: Vec<&str> = value.non_null_field_names();
                let mut first = true;
                let mut sqlx_query = sqlx::QueryBuilder::new(format!("UPDATE {} set ", #tname ));
                #(#inner_field_bindings);* ;
                #(#where_bindings)*;
                println!("Update: {:?}", sqlx_query.sql());
...
fn generate_select_where_impl(table: &parse::Table) -> proc_macro2::TokenStream {
    let tname = table.name.as_str();
    let struct_name = syn::Ident::new(to_camel_case(tname).as_str(), proc_macro2::Span::call_site());
    let primary_keys: Vec<String> = table
        .constraints
    ...
    let cols = table
        ...
        .collect::<Vec<_>>()
        .join(",");
    let select_where = "SELECT ".to_owned() + &cols + " FROM " + tname + " WHERE " + &primary_keys.iter().enumerate().map(|(index, item)| format!("{} = ${}", item, index + 1)).collect::<Vec<String>>().join(" AND ");
...
fn generate_select_impl(table: &parse::Table, select_override: Option<String>) -> proc_macro2::TokenStream {
    let tname = table.name.as_str();
    let struct_name = syn::Ident::new(to_camel_case(tname).as_str(), proc_macro2::Span::call_site());
    let where_clause_bindings = table.columns.iter().enumerate().filter(|col| !col.1.dtype.contains("[]")).map(|(index, col)| {
        let colname = &col.name;
        let field_name = syn::Ident::new(&col.name, proc_macro2::Span::call_site());
        let dtype = col.dtype.as_str();
        ...
        match dtype {
            "uuid" => quote! {
                if value.#field_name.is_some() {
                    ...
                    sqlx_query.push(format!(" {} = ",#colname));
                    count+=1;
                    
                    sqlx_query.push_bind( Uuid::parse_str(value.#field_name.as_ref().unwrap()).unwrap()   ); }
            },
            _ => quote! {
                if value.#field_name.is_some() {
                    if count >0{
                    ...
                    sqlx_query.push_bind(value.#field_name.as_ref().unwrap()); }
            },
        }
    });
    let cols = table
        ...
        .collect::<Vec<_>>()
        .join(",");
    let select = match select_override 
    { None => "SELECT ".to_owned() + &cols + " FROM " + tname,
        Some(value) => value,
};
...
#[async_trait]
        impl SelectAll for #struct_name {
            async fn select_all(pool: &PgPool) -> Result<Vec<Self>, sqlx::Error> {
                let mut sqlx_query = sqlx::QueryBuilder::new(#select.to_string());
                println!("{}",#select);
                let final_query: sqlx::query::QueryAs<_,#struct_name,_> = sqlx_query.build_query_as();
        ...
        impl Select for #struct_name {
            async fn select(pool: &PgPool, value:Self) -> Result<Vec<Self>, sqlx::Error> {
                let mut sqlx_query = sqlx::QueryBuilder::new(#select.to_string());
                let mut count = 0;
                if value.where_clause().len() > 0{
                    sqlx_query.push(" where ");
                }
                #(#where_clause_bindings);* ;

                println!("SQL {}:{:?}",count, value.where_clause() );
                let final_query: sqlx::query::QueryAs<_,#struct_name,_> = sqlx_query.build_query_as();
                final_query.fetch_all(pool).await
            }
        }
...
fn generate_fields<T: quote::ToTokens>(
    ...
    fieldst.iter().map(|(field_ty, _, _, field_name, field_name_str)| {
...
fn generate_struct_minimal(
    struct_name: &syn::Ident,
    fields: impl Iterator<Item = proc_macro2::TokenStream>,
) -> proc_macro2::TokenStream {
    quote! {
               #[derive(Deserialize,Serialize,Debug,Clone,sqlx::FromRow,JsonSchema )]
...
#[proc_macro]
pub fn from_view(attr: TokenStream) -> TokenStream {
    let relative_path: String = "schema.sql".to_string();
    let table_vec = parse::parse_ddl_file(relative_path);

    let sql = parse_macro_input!(attr as LitStr).value();

let view: parse::View = match parse::create_view_parser().parse(&sql.as_str()){
        ...
        let mut table_hash = std::collections::HashMap::new();
        for table in table_vec{
            table_hash.insert(table.0.name.clone(), table.0.clone());
        }
        let mut hierachy = parse::build_table_hierarchy(&view.select, &table_hash);

        hierachy.print();
    
    let t: parse::Table = parse::Table::from_view(&view, table_hash);
    let fieldst = t.map_columns_to_fields();
        let fields = generate_fields(&fieldst);
let struct_name = syn::Ident::new(&t.name, proc_macro2::Span::call_site());

    let statement = sql.to_lowercase().replace("create view", "").replace(&t.name, "");

        let struc = generate_struct(&struct_name, fields);
    println!("{}", struc);
    let basic = generate_basic_impl(&t);
    let imp = generate_select_impl(&t, Some(statement.to_string()) );
